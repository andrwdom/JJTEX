...
const categorySchema = new mongoose.Schema({
	...
	slug: { type: String, required: true, index: false }, // Explicitly disable auto-index to prevent duplicate
	description: String,
	image: String,
...
// Note: Only define indexes explicitly here, not in schema fields
categorySchema.index({ path: 1 }, { unique: true, sparse: true });
categorySchema.index({ parent: 1, order: 1 });
categorySchema.index({ name: 1, parent: 1 }, { unique: true, sparse: true });
// Compound index on slug+parent (this is the only index involving slug)
